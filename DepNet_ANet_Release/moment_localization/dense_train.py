...
if __name__ == '__main__':
    ...
    def network(sample):
        ...
        prediction, map_mask = model(textual_input, textual_mask, sentence_mask, visual_input)
        loss_value, loss_overlap, loss_order, joint_prob = getattr(loss, config.LOSS.NAME)(prediction, map_mask, sentence_mask, map_gt, config.LOSS.PARAMS)

        sorted_times = None if model.training else get_proposal_results(joint_prob, duration)

        return loss_value, loss_overlap, loss_order, sorted_times
